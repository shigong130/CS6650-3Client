...
public class Worker implements Runnable {
    ...
    public void sendPostRequest(boolean sentGetRequest){
        ...
        try {
            ...
            if(sentGetRequest) {
                try {
                    apiInstance.getSkierDayVerticalWithHttpInfo(RESORT_ID, SEASON_ID, DAY_ID, skierId);
                }catch (Exception e){
                    //logger.info("get failed" + e.toString());
                }
            }
    ...
    public static void main(String[] args){
        for(int i=1; i<=100; i++) {
            ...
            //String ec = "http://18.208.162.31:8080/distributedSystemClass_war/";
            //String url = "http:localhost:8080/SkiResortsServer_war_exploded";
            String ec2_3="http://3.87.48.236:8080/distributedSystemClass_war/";

            Worker wk = new Worker("1", 1, 1, 1, i, i, 1, 1, null,
                    new AtomicInteger(0), new AtomicInteger(0), new ArrayList<HttpRecord>(), false, ec2_3);
            new Thread(wk).start();
        }
    }
}
